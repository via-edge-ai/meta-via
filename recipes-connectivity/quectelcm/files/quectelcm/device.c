...
static int detect_network_iface(void *ptr, const char *d_name)
{
    ...
    // ifaceNum 6
    snprintf(pl->path, sizeof(pl->path), "%s/%s:1.6", rootdir, d_name);
    if (!access(pl->path, R_OK))
    {
        snprintf(pl->path, sizeof(pl->path), "%s/%s:1.6/GobiQMI", rootdir, d_name);
        if (!access(pl->path, R_OK))
            return 0;

        snprintf(pl->path, sizeof(pl->path), "%s/%s:1.6/usbmisc", rootdir, d_name);
        if (!access(pl->path, R_OK))
            return 0;

        snprintf(pl->path, sizeof(pl->path), "%s/%s:1.6/usb", rootdir, d_name);
        if (!access(pl->path, R_OK))
            return 0;
    }
    ...
    dbg_time("cannot find GobiQMI/usbmic/usb in path %s, subdir %s:1.4/1.6 or %s:1.8", rootdir, d_name, d_name);
    return -1;
}
...
BOOL qmidevice_detect(char *qmichannel, char *usbnet_adapter, unsigned bufsize, int *pbusnum, int *pdevnum) {
    ...
    while ((ent = readdir(pDir)) != NULL)  {
        ...
        if (idVendor == 0x2c7c && idProduct == 0x030a)
            snprintf(pl->path, sizeof(pl->path), "%s/%s:1.6/net", rootdir, ent->d_name);
        else
